"""
We assume that the height of the contours will let us know which onss are numbers or not.
We start with the contor of greatest height

We then add the width for all contour areas that have are inbetween that one

once we have all contours we can get the location to crop to for ocr

if ocr fails because we got chipstack instead, then remove
"""
...
OCR_MODE = PSM.SINGLE_WORD
OCR_MODE = PSM.SINGLE_LINE
...
def get_number(position, cnts_ordered_by_height, image):
    img3 = cv2.cvtColor(np.array(image), cv2.COLOR_BGR2RGB)  # Convert pil iimage to opencv image
    ...
    img = cv2.cvtColor(np.array(image), cv2.COLOR_BGR2RGB)  # Convert pil iimage to opencv image
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

    _, img = cv2.threshold(img, THRESHOLD, 255, cv2.THRESH_BINARY_INV)

    img_pil = Image.fromarray(img)  # Convert numpy cv image to pil image
...
def swc_ocr(img_orig):
    ...
    # fatten contour lines
    cv2.drawContours(img, cnts, -1, (255, 0, 0), SELECTION_CONTOUR_THICKNESS)
    #cv2.imshow("img", img)

    img2 = cv2.cvtColor(np.array(image), cv2.COLOR_BGR2RGB)  # Convert pil iimage to opencv image
